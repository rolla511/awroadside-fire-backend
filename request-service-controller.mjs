...
export function createRequestServiceController({ cacheRoot, fallbackApiBaseUrl = "", fallbackApiStyle = LOCAL_RUNTIME_API_STYLE }) {
  ...
  async function listRequests(helpers) {
    const apiStyle = getApiStyle();
    if (useLocalRoadsideBackend()) {
      const requests = (await helpers.readRequestLog()).map(normalizeRoadsideRequest);
  ...
  async function createRequest(body, helpers) {
    ...
    if (useLocalRoadsideBackend()) {
      const mappedPayload = mapCreatePayload(apiStyle, payload);
      const normalized = typeof helpers.normalizeServiceRequest === "function"
        ? helpers.normalizeServiceRequest(mappedPayload)
        : mappedPayload;
      const created = normalizeCreatedPayload(apiStyle, {
        request: await helpers.createServiceRequest(normalized)
      });
  ...
  async function getRequest(requestId, helpers) {
    if (useLocalRoadsideBackend()) {
      ...
      const request = requests.find((entry) => String(entry.id || entry.requestId) === String(requestId));
      if (!request) {
  ...
  async function applyProviderAction(requestId, action, body, helpers) {
    ...
    if (useLocalRoadsideBackend()) {
      const localPayload = mapProviderActionPayload(apiStyle, payload);
      const updatedRequest = await helpers.applyLocalRequestAction(requestId, action, localPayload);
      const normalized = normalizeActionPayload(apiStyle, { request: updatedRequest }, requestId, payload.action);
