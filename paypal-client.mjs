...
export async function createPayoutBatch(payoutDetails = {}) {
  ...
  const response = await paypalFetch(`${PAYPAL_API_BASE_URL}/v1/payments/payouts`, {
    method: "POST",
    headers: buildJsonHeaders(token, {
...
function requireCredentials() {
  if (!PAYPAL_CLIENT_ID || !PAYPAL_CLIENT_SECRET) {
    throw new Error("Missing PayPal credentials. Set PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET, or set SECRET_KEY_1 as the PayPal client secret.");
  }
}

function normalizeEnvironment(value) {
  return readEnvValue(value).toLowerCase() === "sandbox" ? "sandbox" : "live";
}
