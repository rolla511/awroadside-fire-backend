...
const DEFAULT_RENDER_BACKEND_URL = 'https://awroadside-fire-backend.onrender.com';
 ...
 */
const DEFAULT_BACKEND_URL = DEFAULT_RENDER_BACKEND_URL;

const topNavItems = [
  { id: 'overview', label: 'Home' },
  { id: 'guest', label: 'Get Help' },
  { id: 'subscriber', label: 'Member' },
];
...
const providerAssessmentQuestions = [
  {
    id: 'jumpstartProcedure',
    label: 'Jumpstart safety',
    placeholder: 'Explain how you safely perform a jumpstart.',
  },
  {
    id: 'jackPlacement',
    label: 'Jack placement',
    placeholder: 'Describe where the jack should be placed.',
  },
  {
    id: 'specialtyVehicleJack',
    label: 'Specialty vehicle jack',
    placeholder: 'What type of jack do you use on BMW, van, truck, or Benz platforms?',
  },
  {
    id: 'spoolDefinition',
    label: 'Spool definition',
    placeholder: 'Explain what a spool is in roadside work.',
  },
  {
    id: 'frozenLugNut',
    label: 'Frozen lug nut',
    placeholder: 'Describe how you remove a frozen lug nut safely.',
  },
  {
    id: 'lockoutTools',
    label: 'Lockout tools',
    placeholder: 'List the tools you use for a lockout.',
  },
  {
    id: 'lockoutDamagePrevention',
    label: 'Damage prevention',
    placeholder: 'How do you prevent vehicle damage during a lockout?',
  },
  {
    id: 'incorrectLockoutDamage',
    label: 'Lockout damage risk',
    placeholder: 'What damage can happen if a lockout is done incorrectly?',
  },
  {
    id: 'tirePlugKnowledge',
    label: 'Tire plug knowledge',
    placeholder: 'Describe your experience with plugging a tire.',
  },
  {
    id: 'severeDamageDecision',
    label: 'Severe damage decision',
    placeholder: 'If a repair may cause serious damage, explain how you inform the customer and stop the work.',
  },
];
...
const initialSubscriberSignup = {
  ...
  billingZip: '',
  termsAccepted: false,
  subscriberTermsAccepted: false,
  dispatchOnlyLiabilityAccepted: false,
...
const initialProviderSignup = {
  ...
  experience: '',
  assessmentAnswers: {
    jumpstartProcedure: 'Use approved cables, confirm battery polarity, keep clamps separated, and start with the donor vehicle off before following the manufacturer sequence.',
    jackPlacement: 'Use the manufacturer-designated lift points on level ground with the parking brake set and wheel support in place.',
    specialtyVehicleJack: 'Use the correct low-profile or specialty jack and adapters recommended for the vehicle platform.',
    spoolDefinition: 'A spool is a lift-point or adapter interface used to support some vehicle platforms safely.',
    frozenLugNut: 'Use penetrating oil, the correct socket, steady leverage, and stop before damaging the stud or wheel.',
    lockoutTools: 'Air wedge, long-reach tool, protective sleeve, light, and vehicle-safe lockout tools.',
    lockoutDamagePrevention: 'Protect the frame and glass, control tool pressure, and verify the unlock path before moving the tool.',
    incorrectLockoutDamage: 'Incorrect lockouts can bend trim, scratch paint, break glass, damage weather seals, or harm the locking system.',
    tirePlugKnowledge: 'I inspect the puncture, confirm the tire is repairable, ream correctly, insert the plug safely, and verify pressure afterward.',
    severeDamageDecision: 'I inform the customer about the possible damage, do not complete the risky work, and document the condition before closing the service.',
  },
  providerTermsAccepted: false,
  providerLiabilityAccepted: false,
  providerHoldHarmlessAccepted: false,
...
export default function App() {
  ...
  const hasProviderAccess = Boolean(
    auth?.roles?.includes('PROVIDER') || auth?.providerStatus || profile?.providerProfile
  );
  const hasAdminRole = Boolean(auth?.roles?.includes('ADMIN'));
  const isDevelopment = false; // Forced production for market-ready AAB
  const showInternalTools = Boolean(isDevelopment || hasAdminRole || adminSession?.token);
  const navItems = [
    ...topNavItems,
    ...(hasProviderAccess ? [{ id: 'provider', label: 'Provider' }] : []),
    ...(adminSession?.token ? [{ id: 'admin', label: 'Admin' }, { id: 'security', label: 'Security' }] : []),
  ];
  const visibleSubscriberTabs = auth?.roles?.includes('SUBSCRIBER')
    ? subscriberTabs
    : subscriberTabs.filter((item) => item.id === 'access');
  const visibleProviderTabs = hasProviderAccess
    ? providerTabs
    : providerTabs.filter((item) => item.id === 'access' || item.id === 'info');
  const visibleAdminTabs = adminSession?.token
    ? adminTabs
    : adminTabs.filter((item) => item.id === 'access');
  ...
  const serviceConnected = Boolean(frontendConfig);
  const activeRequestStatus = labelUiStatus(
    uiEventMap,
    'requestStatus',
    activeSessionRequest?.status || activeSessionRequest?.completionStatus || 'Not started'
  );
  const shellStatusItems = [
    {
      label: 'API',
      value: serviceConnected ? 'Connected' : 'Pending',
      tone: serviceConnected ? 'success' : 'warning',
    },
    {
      label: 'Payments',
      value: paymentConfig?.mode || frontendConfig?.paypalMode || 'Unavailable',
      tone: paymentConfig?.mode || frontendConfig?.paypalMode ? 'info' : 'warning',
    },
    {
      label: 'Security',
      value: securityStatus?.watchdog?.integrityOk ? 'Ready' : 'Check',
      tone: securityStatus?.watchdog?.integrityOk ? 'success' : 'warning',
    },
    {
      label: 'Session',
      value: auth?.sessionToken ? prettifyUiToken((auth.roles || ['user']).join(' ')) : 'Guest',
      tone: auth?.sessionToken ? 'neutral' : 'info',
    },
  ];
  ...
  async function handleSignin() {
    ...
    try {
      ...
      const nextProfile = await loadProfile(frontendConfig, nextAuth.sessionToken);
      const inferredRoles = new Set(nextAuth.roles || []);
      if (nextAuth.providerStatus || nextProfile?.providerProfile) {
        inferredRoles.add('PROVIDER');
      }
      if (nextAuth.subscriberActive || nextProfile?.subscriberProfile) {
        inferredRoles.add('SUBSCRIBER');
      }
      const resolvedAuth = {
        ...nextAuth,
        roles: Array.from(inferredRoles),
      };
      if (resolvedAuth.roles.length === 0) {
        throw new Error('Sign-in did not return a usable account role.');
      }
      setAuth(resolvedAuth);
      if (resolvedAuth.roles.includes('PROVIDER')) {
        await loadRequestQueue(frontendConfig, nextAuth.sessionToken);
        setSection('provider');
        setProviderView(nextProfile?.providerProfile ? 'work' : 'info');
      } else if (resolvedAuth.roles.includes('SUBSCRIBER')) {
        setSection('subscriber');
        setSubscriberView('profile');
      } else {
        throw new Error('This account is not configured for the current app flow.');
      }
  ...
  async function handleSubscriberSignup() {
    if (!subscriberSignup.termsAccepted || !subscriberSignup.subscriberTermsAccepted) {
      setErrorMessage('You must accept the terms of service and subscriber terms.');
      return;
    }
    if (!subscriberSignup.dispatchOnlyLiabilityAccepted || !subscriberSignup.noRefundPolicyAccepted) {
      setErrorMessage('You must accept the liability and refund policies.');
      return;
    }
    if (subscriberSignup.password !== subscriberSignup.confirmPassword) {
    ...
    try {
      const signup = await api.signup({
        ...
        phoneNumber: subscriberSignup.phoneNumber,
        username: subscriberSignup.username,
        email: subscriberSignup.email,
        password: subscriberSignup.password,
        confirmPassword: subscriberSignup.confirmPassword,
        role: 'SUBSCRIBER',
        paymentMethodMasked: subscriberSignup.paymentMethodMasked,
        termsAccepted: subscriberSignup.termsAccepted,
        subscriberTermsAccepted: subscriberSignup.subscriberTermsAccepted,
        dispatchOnlyLiabilityAccepted: subscriberSignup.dispatchOnlyLiabilityAccepted,
        noRefundPolicyAccepted: subscriberSignup.noRefundPolicyAccepted,
        subscriberTermsAcceptedAt: new Date().toISOString(),
      });
      ...
      await api.setupSubscriber(
        {
          ...
          paymentMethodMasked: subscriberSignup.paymentMethodMasked,
          billingZip: subscriberSignup.billingZip,
          subscriberTermsAccepted: subscriberSignup.subscriberTermsAccepted,
          subscriberTermsAcceptedAt: new Date().toISOString(),
          dispatchOnlyLiabilityAccepted: subscriberSignup.dispatchOnlyLiabilityAccepted,
          noRefundPolicyAccepted: subscriberSignup.noRefundPolicyAccepted,
  ...
  async function handleProviderSignup() {
    if (!providerSignup.providerTermsAccepted || !providerSignup.providerLiabilityAccepted || !providerSignup.providerHoldHarmlessAccepted) {
      setErrorMessage('You must accept all provider terms and liability agreements.');
      return;
    }
    if (providerSignup.password !== providerSignup.confirmPassword) {
      setErrorMessage('Passwords do not match.');
      return;
    }
    ...
    try {
      const signup = await api.signup({
        ...
        phoneNumber: providerSignup.phoneNumber,
        username: providerSignup.username,
        email: providerSignup.email,
        password: providerSignup.password,
        confirmPassword: providerSignup.confirmPassword,
        role: 'PROVIDER',
        termsAccepted: providerSignup.providerTermsAccepted,
        providerTermsAccepted: providerSignup.providerTermsAccepted,
        providerLiabilityAccepted: providerSignup.providerLiabilityAccepted,
        providerHoldHarmlessAccepted: providerSignup.providerHoldHarmlessAccepted,
      ...
      await api.applyProvider(
        {
          providerInfo: {
            legalName: providerSignup.fullName,
            phoneNumber: providerSignup.phoneNumber,
            email: providerSignup.email,
            companyName: providerSignup.companyName,
          },
          vehicleInfo: {
          ...
          serviceArea: providerSignup.serviceArea,
          currentLocation: providerSignup.currentLocation,
          hoursOfService: buildHoursOfService(providerSignup.availabilityWindow, providerSignup.timezone),
          assessmentAnswers: providerSignup.assessmentAnswers,
          providerTermsAccepted: providerSignup.providerTermsAccepted,
          providerTermsAcceptedAt: new Date().toISOString(),
          providerLiabilityAccepted: providerSignup.providerLiabilityAccepted,
          providerHoldHarmlessAccepted: providerSignup.providerHoldHarmlessAccepted,
  ...
  async function handleCreateRequest() {
    if (!requestForm.fullName || !requestForm.phoneNumber || !requestForm.location || !requestForm.serviceType) {
      setErrorMessage('Full name, phone number, location, and service type are required.');
      return;
    }
    setLoading(true);
    clearMessages();
    try {
      ...
      const displayMessage = error.payload?.message || error.message || 'Something went wrong. Please try again.';
  ...
  function renderShellStatusStrip() {
    return (
      <ScrollView
        horizontal
      ...
      >
        {shellStatusItems.map((item) => (
          <View
            ...
            style={[
              styles.statusChip,
              item.tone === 'success' ? styles.statusChipSuccess : null,
              item.tone === 'warning' ? styles.statusChipWarning : null,
              item.tone === 'info' ? styles.statusChipInfo : null,
            ]}
          >
            <Text style={styles.statusChipLabel}>{item.label}</Text>
            <Text style={styles.statusChipValue}>{item.value}</Text>
          </View>
        ))}
      </ScrollView>
    );
  ...
  function renderSectionBanner({ eyebrow, title, description, metrics = [] }) {
    return (
      <View style={styles.sectionBanner}>
        <View style={styles.sectionBannerHeader}>
          {eyebrow ? <Text style={styles.sectionEyebrow}>{eyebrow}</Text> : null}
          <Text style={styles.sectionBannerTitle}>{title}</Text>
          <Text style={styles.sectionBannerBody}>{description}</Text>
        </View>
        {metrics.length ? (
          <View style={styles.metricGrid}>
            {metrics.map((metric) => (
              <View key={`${title}-${metric.label}`} style={styles.metricCard}>
  ...
  function renderOverviewScreen() {
    const guestServiceCharge = formatOptionalMoney(getCurrentServiceCharge(pricingSource, 'GUEST'));
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        <View style={styles.heroCard}>
          ...
          <View style={styles.heroCopy}>
            <Text style={styles.eyebrow}>24/7 Dispatch Platform</Text>
            <Text style={styles.title}>Roadside assistance, member access, and provider operations in one live build.</Text>
            <Text style={styles.subtitle}>
              The request flow, quote handling, provider dispatch, and admin controls are already wired to the backend. This UI keeps those flows intact and presents them as a usable market-facing product.
            </Text>
            <View style={styles.heroBadgeRow}>
              <View style={styles.heroBadge}>
                <Text style={styles.heroBadgeLabel}>Guest Dispatch</Text>
                <Text style={styles.heroBadgeValue}>{guestServiceCharge}</Text>
              </View>
              <View style={styles.heroBadge}>
                <Text style={styles.heroBadgeLabel}>Subscriber Plan</Text>
                <Text style={styles.heroBadgeValue}>{formatOptionalMoney(pricingSource?.subscriberMonthlyFee)}</Text>
              </View>
              <View style={styles.heroBadge}>
                <Text style={styles.heroBadgeLabel}>Provider Queue</Text>
                <Text style={styles.heroBadgeValue}>{providerQueue.length}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.metricGrid}>
          <View style={styles.metricCard}>
            <Text style={styles.metricLabel}>Dispatch Status</Text>
            <Text style={styles.metricValue}>{activeRequestStatus}</Text>
          </View>
          <View style={styles.metricCard}>
            <Text style={styles.metricLabel}>Payment Mode</Text>
            <Text style={styles.metricValue}>{paymentConfig?.mode || frontendConfig?.paypalMode || 'Unavailable'}</Text>
          </View>
          <View style={styles.metricCard}>
            <Text style={styles.metricLabel}>Member Requests</Text>
            <Text style={styles.metricValue}>{subscriberRequestHistory.length}</Text>
          </View>
          <View style={styles.metricCard}>
            <Text style={styles.metricLabel}>Security</Text>
            <Text style={styles.metricValue}>{securityStatus?.watchdog?.integrityOk ? 'Ready' : 'Check'}</Text>
          </View>
        </View>
        ...
        <View style={styles.panel}>
          <Text style={styles.sectionTitle}>Core Service Types</Text>
          <Text style={styles.mutedText}>Choose the needed service first, then move through quote, payment, and dispatch confirmation from the same app session.</Text>
          <View style={styles.serviceGrid}>
            {serviceOptions.map((service) => (
              <ServiceCard
                ...
                label={service.label}
                price={guestServiceCharge}
                detail={service.detail}
                onPress={() => {
                  setRequestForm((current) => ({ ...current, serviceType: service.id }));
                  setSection('guest');
        ...
        <View style={styles.quickGrid}>
          <Pressable style={styles.quickCard} onPress={() => setSection('guest')}>
            <Text style={styles.quickLabel}>Get Help</Text>
            <Text style={styles.quickBody}>Start a guest request, review the live quote, and keep status tracking inside this session.</Text>
          </Pressable>
          <Pressable style={styles.quickCard} onPress={() => setSection('subscriber')}>
            <Text style={styles.quickLabel}>Member Access</Text>
            <Text style={styles.quickBody}>Sign in, manage vehicle details, and use the subscriber request and confirmation flow.</Text>
          </Pressable>
          <Pressable
            style={styles.quickCard}
            onPress={() => {
              setSection('provider');
              setProviderView('info');
            }}
          >
            <Text style={styles.quickLabel}>Provider Network</Text>
            <Text style={styles.quickBody}>Apply, complete the safety intake, and move into queue, log, and wallet screens after approval.</Text>
          </Pressable>
          {showInternalTools ? (
            <Pressable style={styles.quickCard} onPress={() => setSection('admin')}>
              <Text style={styles.quickLabel}>Admin Access</Text>
              <Text style={styles.quickBody}>Open the restricted control surface for approvals, refunds, payouts, and security review.</Text>
            </Pressable>
          ) : null}
        </View>
      </ScrollView>
  ...
  function renderGuestScreen() {
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        {renderSectionBanner({
          eyebrow: 'Guest Dispatch',
          title: 'One-time roadside service',
          description: 'Create a guest request, confirm the quoted charge, and track the active dispatch state without signing in.',
          metrics: [
            { label: 'Current Request', value: activeRequestId || 'None' },
            {
              label: 'Quote',
              value: servicePaymentQuote?.amount?.value
                ? `${servicePaymentQuote.amount.value} ${servicePaymentQuote.amount.currency_code}`
                : 'Pending',
            },
            {
              label: 'Order',
              value: labelUiStatus(
                uiEventMap,
                'paymentStatus',
                paymentOrder?.status || (paymentOrder?.captured ? 'CAPTURED' : 'Not started')
              ),
            },
          ],
        })}
    ...
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        {renderSectionBanner({
          ...
          title: 'Member profile, requests, and confirmations',
          description: 'The member flow keeps vehicle details, billing status, request history, and post-dispatch confirmations in one place.',
          metrics: [
            { label: 'Session', value: auth?.sessionToken ? 'Signed in' : 'Guest' },
            { label: 'Membership', value: profile?.subscriberActive || auth?.subscriberActive ? 'Active' : 'Inactive' },
            { label: 'History', value: String(subscriberRequestHistory.length) },
          ],
        })}
        {renderTabBar(visibleSubscriberTabs, subscriberView, setSubscriberView)}
    ...
    return (
      <>
        ...
        <View style={styles.panel}>
          ...
          <Text style={styles.mutedText}>Create your account, confirm your vehicle, and approve the member terms before activation.</Text>
    ...
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        {renderSectionBanner({
          ...
          title: 'Queue, service log, and payout visibility',
          description: 'Provider screens keep the intake, live queue, assignment notes, and payout state close to the work itself.',
          metrics: [
            { label: 'Status', value: labelUiStatus(uiEventMap, 'providerStatus', profile?.providerStatus || auth?.providerStatus || 'Not signed in') },
            { label: 'Open Queue', value: String(providerQueue.length) },
            { label: 'Assigned', value: String(providerOwnedRequests.length) },
          ],
        })}
        {renderTabBar(visibleProviderTabs, providerView, setProviderView)}
  ...
  function renderProviderInfoScreen() {
    return (
      <View style={styles.panel}>
        ...
        <Text style={styles.mutedText}>Complete the provider intake, safety review, and document readiness checks before the application is sent.</Text>
    ...
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        {renderSectionBanner({
          ...
          title: 'Account review, dispatch oversight, and financial actions',
          description: 'Admin views stay focused on queue health, provider approvals, subscriber context, refunds, payouts, and security refreshes.',
          metrics: [
            { label: 'Dashboard', value: adminDashboard ? 'Loaded' : 'Pending' },
            { label: 'Queue', value: String(adminDashboard?.queue?.length ?? 0) },
            { label: 'Pending Providers', value: String(adminDashboard?.stats?.pendingProviders ?? 0) },
          ],
        })}
        {renderTabBar(visibleAdminTabs, adminView, setAdminView)}
  ...
  function renderSecurityScreen() {
    return (
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        {renderSectionBanner({
          eyebrow: 'Security Runtime',
          title: 'Watchdog integrity and platform endpoints',
          description: 'This screen exposes the current watchdog state, active runtime layer, and the backend URLs the mobile build is reading from.',
          metrics: [
            { label: 'Layer', value: securityStatus?.layer || securityStatus?.watchdog?.layer || 'Unknown' },
            { label: 'Integrity', value: securityStatus?.watchdog?.integrityOk ? 'Ready' : 'Check' },
            { label: 'Variant', value: compatibilityVariant?.mode || compatibilityManifest?.mode || 'Unverified' },
          ],
        })}
        <View style={styles.panel}>
  ...
  function renderRequestStatusCard(request) {
    const requestId = request.requestId || request.id;
    const requestStatus = normalizeValue(request.status || request.completionStatus);
    const requestStageIndex = getRequestTimelineIndex(requestStatus);
    return (
      <View key={`session-${requestId}`} style={styles.queueCard}>
        ...
        <View style={styles.progressRow}>
          {requestTimelineSteps.map((step, index) => {
            const completed = requestStageIndex >= index;
            const active = requestStageIndex === index;
            return (
              <View
                key={`${requestId}-${step.id}`}
                style={[
                  styles.progressPill,
                  completed ? styles.progressPillComplete : null,
                  active ? styles.progressPillActive : null,
                ]}
              >
                <Text
                  style={[
                    styles.progressPillLabel,
                    completed ? styles.progressPillLabelComplete : null,
                  ]}
                >
                  {step.label}
                </Text>
        ...
        <Text style={styles.mutedText}>Location: {request.location || 'Location pending'}</Text>
        ...
        <Text style={styles.mutedText}>Acceptance window ends: {formatDate(request.requestAcceptanceExpiresAt)}</Text>
        {requestStatus === 'EXPIRED' ? (
          <Text style={styles.mutedText}>No provider accepted this request within the five-minute assignment window.</Text>
        ) : null}
      ...
      <View style={styles.modalSimFull}>
        <ScrollView style={styles.modalContent}>
          ...
          <Text style={styles.bodyText}>
            By using awroadside_fire_live, you agree to our Terms of Service and Privacy Policy.
  ...
  function renderSwitchRow(label, value, onValueChange) {
    return (
      <View style={styles.switchRow}>
        ...
        <Switch
          ...
          thumbColor={Boolean(value) ? theme.colors.gold : theme.colors.text}
        />
      </View>
    );
  }
  ...
  function renderSubscriberSignupFields() {
    return (
      <View style={styles.fieldStack}>
        ...
        <InputField label="Phone Number" value={subscriberSignup.phoneNumber} onChangeText={(value) => setSubscriberSignup((current) => ({ ...current, phoneNumber: value }))} />
        ...
        <InputField label="Confirm Password" secureTextEntry value={subscriberSignup.confirmPassword} onChangeText={(value) => setSubscriberSignup((current) => ({ ...current, confirmPassword: value }))} />
        ...
        <InputField label="Billing ZIP" value={subscriberSignup.billingZip} onChangeText={(value) => setSubscriberSignup((current) => ({ ...current, billingZip: value }))} />
        {renderSwitchRow('Accept platform terms of service', subscriberSignup.termsAccepted, (value) =>
          setSubscriberSignup((current) => ({ ...current, termsAccepted: value }))
        )}
        {renderSwitchRow('Accept subscriber terms', subscriberSignup.subscriberTermsAccepted, (value) =>
          setSubscriberSignup((current) => ({ ...current, subscriberTermsAccepted: value }))
        )}
        {renderSwitchRow('Accept dispatch-only liability terms', subscriberSignup.dispatchOnlyLiabilityAccepted, (value) =>
          setSubscriberSignup((current) => ({ ...current, dispatchOnlyLiabilityAccepted: value }))
        )}
        {renderSwitchRow('Accept no-refund policy', subscriberSignup.noRefundPolicyAccepted, (value) =>
          setSubscriberSignup((current) => ({ ...current, noRefundPolicyAccepted: value }))
        )}
        <Pressable onPress={() => setShowTerms(true)} style={styles.textLink}>
          <Text style={styles.textLinkLabel}>View member terms</Text>
        </Pressable>
      </View>
    );
  }
  ...
  function renderProviderSignupFields() {
    return (
      <View style={styles.fieldStack}>
        ...
        <InputField label="Phone Number" value={providerSignup.phoneNumber} onChangeText={(value) => setProviderSignup((current) => ({ ...current, phoneNumber: value }))} />
        ...
        <InputField label="Confirm Password" secureTextEntry value={providerSignup.confirmPassword} onChangeText={(value) => setProviderSignup((current) => ({ ...current, confirmPassword: value }))} />
        <InputField label="Company Name" value={providerSignup.companyName} onChangeText={(value) => setProviderSignup((current) => ({ ...current, companyName: value }))} />
        <InputField label="Service Area" value={providerSignup.serviceArea} onChangeText={(value) => setProviderSignup((current) => ({ ...current, serviceArea: value }))} />
        <InputField label="Current Location" value={providerSignup.currentLocation} onChangeText={(value) => setProviderSignup((current) => ({ ...current, currentLocation: value }))} />
        <InputField label="Availability Window" value={providerSignup.availabilityWindow} onChangeText={(value) => setProviderSignup((current) => ({ ...current, availabilityWindow: value }))} />
        <InputField label="Timezone" value={providerSignup.timezone} onChangeText={(value) => setProviderSignup((current) => ({ ...current, timezone: value }))} />
        ...
        {renderSwitchRow('License ready to submit', providerSignup.license, (value) =>
          setProviderSignup((current) => ({ ...current, license: value }))
        )}
        {renderSwitchRow('Registration ready to submit', providerSignup.registration, (value) =>
          setProviderSignup((current) => ({ ...current, registration: value }))
        )}
        {renderSwitchRow('Insurance ready to submit', providerSignup.insurance, (value) =>
          setProviderSignup((current) => ({ ...current, insurance: value }))
        )}
        {renderSwitchRow('Helper ID available', providerSignup.helperId, (value) =>
          setProviderSignup((current) => ({ ...current, helperId: value }))
        )}
        ...
        <Text style={styles.label}>Safety Assessment</Text>
        {providerAssessmentQuestions.map((question) => (
          <InputField
            key={question.id}
            label={question.label}
            multiline
            value={providerSignup.assessmentAnswers[question.id] || ''}
        ...
        {renderSwitchRow('Accept provider terms', providerSignup.providerTermsAccepted, (value) =>
          setProviderSignup((current) => ({ ...current, providerTermsAccepted: value }))
        )}
        {renderSwitchRow('Accept provider liability acknowledgement', providerSignup.providerLiabilityAccepted, (value) =>
          setProviderSignup((current) => ({ ...current, providerLiabilityAccepted: value }))
        )}
        {renderSwitchRow('Accept provider hold harmless policy', providerSignup.providerHoldHarmlessAccepted, (value) =>
          setProviderSignup((current) => ({ ...current, providerHoldHarmlessAccepted: value }))
        )}
        <Pressable onPress={() => setShowTerms(true)} style={styles.textLink}>
          <Text style={styles.textLinkLabel}>View provider terms</Text>
        </Pressable>
      </View>
    );
  }
      ...
      >
        <View style={styles.header}>
          <View style={styles.brandBlock}>
            <Text style={styles.brand}>awroadside_fire_live</Text>
            <Text style={styles.headerMeta}>Roadside support, member access, provider tools, and admin controls tied to the live backend.</Text>
          </View>
...
function buildHoursOfService(windowLabel, timezone) {
  const normalizedWindow = typeof windowLabel === 'string' && windowLabel.trim() ? windowLabel.trim() : '24/7';
  const normalizedTimezone = typeof timezone === 'string' && timezone.trim() ? timezone.trim() : 'America/New_York';
  return {
    timezone: normalizedTimezone,
    monday: normalizedWindow,
    tuesday: normalizedWindow,
    wednesday: normalizedWindow,
    thursday: normalizedWindow,
    friday: normalizedWindow,
    saturday: normalizedWindow,
    sunday: normalizedWindow,
  };
}
...
function getRequestTimelineIndex(status) {
  const normalizedStatus = normalizeValue(status);
  const matchedIndex = requestTimelineSteps.findIndex((step) => step.id === normalizedStatus);
  if (matchedIndex >= 0) {
    return matchedIndex;
  }
