 ...
 * Mobile baseline compared against: legacy mobile App.js snapshot
 *
...
const fireDoubleCompatibility = Object.freeze({
  baseRuntime: "web",
  comparedMobileSource: "legacy mobile App.js snapshot",
  sharedFlows: [
    "bootstrap",
    "signin",
    "subscriber-signup",
...
function resolveAdminApiBaseUrl(value) {
  const fallback = normalizeUrlValue(runtimeConfig.adminApiBaseUrl) || `${window.location.origin}/admin-controller.mjs`;
  const candidate = normalizeUrlValue(value);
  if (!candidate) {
    return fallback;
  }
